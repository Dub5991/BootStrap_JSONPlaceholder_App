...
function Users() {
  const [users, setUsers] = useState([]);     // State to store users in an array
  const [loading, setLoading] = useState(true); // Loading state to show spinner
  const [error, setError] = useState(null);    // Error state to show error message. Null means no error
      ...
      <Row>
        {users.map(user => (
          <Col key={user.id} className="mt-4">
            <Card style={{ width: '18rem' }} > {/* rem is a unit of measurement relative to the font-size of the root element*/}
              <Card.Body>
